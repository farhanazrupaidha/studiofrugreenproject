...
const FLORA = gql`
  query {
      posts(where: {tags_contains_all: "Wisata"}, orderBy: date_DESC, first: 4) {
        title
        slug
        excerpt
        date
...
<Box>
<Typography variant="h3" sx={{mb:5}}>Wisata</Typography>    
  <Stack spacing={{ xs: 2, sm: 2 }} direction="row" useFlexGap flexWrap="wrap" justifyContent="center"> 
    {data &&
      data.posts.map((item) => (
        <Box key={item.id} sx={{maxWidth:200, width:'100%', mb:2}}>       
          <div className="mb-5">
