...
export async function getPostAndMorePosts(slug, preview) {
  const data = await fetchAPI(
    ...
    query PostBySlug($slug: String!, $stage: Stage!) {
      ...
      morePosts: posts(locales: id, orderBy: date_DESC, first: 6, where: {slug_not_in: [$slug]}) {
        title
        slug
        excerpt
        date
...
export async function getPostAndMorePostsEn(slug, preview) {
  const data = await fetchAPI(
    ...
    query PostBySlug($slug: String!, $stage: Stage!) {
      ...
      morePosts: posts(locales: en, orderBy: date_DESC, first: 6, where: {slug_not_in: [$slug]}) {
        title
        slug
        excerpt
        date
